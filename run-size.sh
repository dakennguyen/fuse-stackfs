...
iosizes=(4k)
sizes=(4k 32k 1m 16m 64m 128m 256m 512m 1g)
...
# Output CSV
csv_file="${template_name}-sizes.csv"
echo "filesystem,size,ops,total_ops_per_sec,read_ops,write_ops,throughput,latency,avg_usr,avg_sys,avg_util" > "$csv_file"

# Loop over all combinations
for dir in "${dirs[@]}"; do
  for size in "${sizes[@]}"; do
    echo "=== Running workload: dir=$dir, size=$size ==="
    ...
    if [[ -s tmp_summary.txt ]]; then
      ...
      if [[ $summary_line =~ ([0-9\.]+):\ IO\ Summary:\ *([0-9]+)\ ops\ ([0-9\.]+)\ ops/s\ ([0-9]+)/([0-9]+)\ rd/wr\ *([0-9\.a-zA-Z/]+)\ +([0-9\.]+)ms/op ]]; then
        ...
        echo "${dir##*/},$size,$ops,$ops_per_sec,$rd_ops,$wr_ops,$throughput,$latency,$stats" >> "$csv_file"
      else
        echo "${dir##*/},$size,PARSE_ERROR,,,,,,$stats" >> "$csv_file"
      fi
    else
      echo "${dir##*/},$size,NO_SUMMARY,,,,,,$stats" >> "$csv_file"
    fi
